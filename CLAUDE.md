...
- `protea.training` → export_research_dataset (serialized; GPU/RAM-intensive KNN + feature generation + artifact-store upload) **or** export_coordinator (when `PROTEA_EXPORT_MINIJOBS=1`). Re-ranker *training* itself no longer runs in PROTEA: see "Re-ranker training decoupling" below.
- `protea.training.knn-batch` → export_knn_batch (OperationConsumer, no DB Job row; env-gated `PROTEA_EXPORT_MINIJOBS=1`)
- `protea.training.features` → export_features_batch (OperationConsumer; env-gated)
- `protea.training.write` → export_write (OperationConsumer; env-gated)
