    ...
    <>
      ...
      <ContextBanner
        ...
        prerequisites={proteinCount !== null ? [
          { label: `${proteinCount} proteins loaded`, met: proteinCount > 0, href: "/instrument/proteins" },
          { label: `${configs.length} embedding config(s)`, met: configs.length > 0 },
        ] : undefined}
        nextStep={{ label: "Functional Annotation", href: "/instrument/functional-annotation" }}
      />
              ...
              <form onSubmit={handleComputeSubmit} className="space-y-4">
                ...
                {cmpResult && (
                  <div className="rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">
                    Job queued:{" "}
                    <Link href={`/${locale}/instrument/jobs/${cmpResult.id}`} className="font-mono underline hover:text-green-900">
                      {cmpResult.id}
                    </Link>
                  </div>
                )}
