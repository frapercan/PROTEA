 ...
 *
 * Every value below is read from the benchmark's own frozen result mirror, the
 * `predictions_protea.tsv` row of
 * `CAFA_forever/data/releases/Sep_2025_Mar_2026/results_{NK,LK,PK}/evaluation_best_f_micro_w.tsv`,
 * and regenerates in one command via `storage/regen_headline/board_nine_cell.py`.
 * `won` compares against the EXTERNAL field only: PROTEA-percut-graft,
 * PROTEA-reranked and PROTEA-KNN are our own variants and are excluded by the
 * release's `method_names.tsv`, never by a filename guess, because
 * `predictions_percutgraft.tsv` carries no "protea" substring and a guess counts
 * our own graft as a rival.
 */
export const NINE_CELL: Record<Knowledge, Record<Aspect, GridCell>> = {
  NK: { MF: { value: 0.6637, won: true }, BP: { value: 0.3374, won: true }, CC: { value: 0.4770, won: true } },
  LK: { MF: { value: 0.5638, won: true }, BP: { value: 0.4402, won: false }, CC: { value: 0.4615, won: true } },
  PK: { MF: { value: 0.2416, won: true }, BP: { value: 0.2181, won: false }, CC: { value: 0.2655, won: true } },
};
...
export const PILLARS: Pillar[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      valueHeader: METRIC,
      rows: [
        { label: "NK molecular function", value: "0.6637" },
        { label: "NK biological process", value: "0.3374", note: "we lead the field here" },
        { label: "NK cellular component", value: "0.4770" },
        { label: "LK molecular function", value: "0.5638" },
        { label: "LK biological process", value: "0.4402", frontier: true, note: "not won: TransFew 0.5120" },
        { label: "LK cellular component", value: "0.4615" },
        { label: "PK molecular function", value: "0.2416" },
        { label: "PK biological process", value: "0.2181", frontier: true, note: "not won: TransFew 0.2943" },
        { label: "PK cellular component", value: "0.2655" },
      ],
    },
    receipt: {
    ...
    caveats: [
      "The nine values are the benchmark's own frozen scoring of our submission, read straight from its result mirror and regenerated by one script. The regime reading on top of them, which evidence carries which cell, comes from a leave-one-family-out ablation whose receipt lives in the lab repository rather than in this page's storage snapshot.",
      "Won or not won is judged against the external field only. Three of the rows on the public board are our own variants, and on four cells one of them edges the submission by less than a thousandth. Counting them as rivals would report two cells won instead of seven, which is a real trap this project has fallen into once.",
