...
def _best(rows: list[dict[str, Any]]) -> float:
    """The best weighted micro F in one panel, an unscored row counting zero."""
...
def _clears(at_floor: list[dict[str, Any]], rivals: list[dict[str, Any]]) -> bool:
    ...
    One expression, read by both the yes-or-no below and the word beside it, so
    that the verdict a comparison is reported in and the reading it is reported
    with cannot drift into disagreeing about what clearing a floor is.
    """
...
def _testimony(
    ...
    Split out of :func:`separated_from_floor` when a second caller appeared that
    needs the same panels for a different question. Two loops over ``PANEL_KEYS``
    would have been two places holding the frame rule and the crossed-depth
    refusal, and a comparison whose two readers disagree about which panels
    testified is the defect this whole surface keeps being bitten by.
...
def reading_against_floor(
    ...
    """The declared comparison's answer, in the panel tally's own six words.

    WHY SIX WORDS BESIDE THE BOOLEAN AND NOT INSTEAD OF IT.
    :func:`separated_from_floor` answers yes or no, and its no is three different
    facts: the panels that testified could have shown the difference this project
    acts on and did not, they could not have shown it, or no panel carried both
    sides and nothing was compared at all. ``compare_paired_panels`` has told
    those apart since it was written -- ``null_with_power``, ``underpowered``,
    ``not_computed`` -- and the graph surface published all three, together with
    a node that declared no floor, as ``chosen``. A null the campaign measured
    was indistinguishable from a question nobody asked.

    The yes is left exactly where it was. This function returns ``resolved`` on
    precisely the panels that make :func:`separated_from_floor` true, because
    both read the same testimony through the same :func:`_clears`, and the edge
    that carries them refuses to hold one without the other.

    THE POWER QUESTION IS THE PANEL'S OWN ARITHMETIC. A panel resolves
    :func:`detectable_effect` of its population and nothing finer. The comparison
    had power when every panel that testified could have resolved
