...
class PredictGOTermsBatchOperation:
    ...
    def _enrich_with_v6_features(
        ...
        *,
        ...
        merge), each stage extracted as its own helper to keep this method
        readable as a high-level orchestrator.
        """
        if not predictions:
            return
        ...
        gtids_in_play = self._collect_gtids_in_play(predictions, go_map_by_aspect)
        go_id_map, go_aspect_map = self._load_go_term_metadata(session, gtids_in_play)

        # ── 2. Anc2Vec projection pool ──────────────────────────────────
        idx_of_go, all_norm, has_emb_mask = self._build_anc2vec_pool(
            gtids_in_play, go_id_map
        )

        # ── 3. Neighbor centroids per (q_acc, aspect) ───────────────────
        neighbor_info = self._compute_neighbor_centroids(
            valid_accessions=valid_accessions,
            neighbors_by_aspect=neighbors_by_aspect,
            go_map_by_aspect=go_map_by_aspect,
            go_id_map=go_id_map,
        ...
        tax = self._compute_tax_voter_counters(
            valid_accessions=valid_accessions,
            neighbors_by_aspect=neighbors_by_aspect,
            go_map_by_aspect=go_map_by_aspect,
            pair_features=pair_features,
        ...
        tax_same_cnt, tax_close_cnt, tax_ca_sum, tax_ca_n, vote_count_div = tax

        # ── 5. PCA query projection ──────────────────────────────────────
        pca_query_proj = self._compute_pca_projection(query_embeddings, pca_state)
    ...
    # ── _enrich_with_v6_features stages ─────────────────────────────────────
