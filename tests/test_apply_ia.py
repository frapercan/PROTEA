...
``protea.core.operations.predict_go_terms._post_knn_pipeline``. The IA-table
resolver and the go_id resolver are mocked so the test exercises only the
...
def _run(prediction_dicts, *, ia_map, gid_by_go):
    """Drive ``apply_ia`` with both DB-bound resolvers mocked."""
    with (
        patch.object(pkp, "_load_ia_feature_map", return_value=ia_map),
        patch.object(pkp, "_load_go_id_and_aspect", return_value=(gid_by_go, {})),
    ):
        pkp.apply_ia(MagicMock(), MagicMock(), prediction_dicts, _emit)
...
def test_empty_ia_map_noops() -> None:
    rec = {"protein_accession": "Q1", "go_term_id": 11}
    with patch.object(pkp, "_load_ia_feature_map", return_value={}):
        pkp.apply_ia(MagicMock(), MagicMock(), [rec], _emit)
...
def test_resolver_priority_env_over_snapshot(tmp_path, monkeypatch) -> None:
    ...
    pkp._cached_ia_map.cache_clear()
    session = MagicMock()
    out = pkp._load_ia_feature_map(session, MagicMock(), None, _emit)
