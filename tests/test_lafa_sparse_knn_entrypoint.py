...
One thing the harness cannot reproduce: outside a container the output
directory is never a bind mount, so the bind-mount style always stops at exit
69. Tests that need to reach past that point use the guide's style, and each
one says so.
"""
...
def test_the_output_path_is_read_from_an_equals_form_too(layout: Layout) -> None:
    ...
    result = layout.run(f"--output={layout.out_arg()}")
    assert result.returncode == 0, result.stderr
...
def test_an_unwritable_output_is_refused_before_any_work(layout: Layout) -> None:
    ...
    Guide style only: in the bind-mount style the mount check answers 69 first,
    and outside a container that check always fires, so a writability test there
