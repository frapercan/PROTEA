...
class TestBuildMetricRegistry:
    def test_returns_registry_with_five_metrics(self) -> None:
        ...
        # All five metrics live on the same CollectorRegistry instance.
        names = {m.describe()[0].name for m in metrics.registry._names_to_collectors.values()}
        # Note: prometheus_client exposes histograms under the base name.
        assert (
            "protea_jobs_total" in {n.removesuffix("_total") + "_total" for n in names}
            or "protea_jobs" in names
        )
        assert "protea_job_duration_seconds" in names
        assert "protea_embeddings_batch_seconds" in names
        assert "protea_predictions_batch_seconds" in names
        assert "protea_db_pool_in_use" in names
